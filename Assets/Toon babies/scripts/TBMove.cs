...
public class TBMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (ControlFreak2.CF2Input.GetKey(KeyCode.C))
            {
                anim.SetInteger("up", 3);
                StartCoroutine("wait", waitime);
            }
            if (ControlFreak2.CF2Input.anyKey)
            {
                anim.SetInteger("up", 4);
                StartCoroutine("wait", waitime);
            }
        ...
        {
            if (ControlFreak2.CF2Input.anyKey) anim.SetInteger("up", 3);
            StartCoroutine("wait", waitime);
        }
        ...
        {
            //CHANGE STATE
            if (ControlFreak2.CF2Input.anyKey) timecounter = 0;
            timecounter++;
            ...
            //CRAWL
            if (ControlFreak2.CF2Input.GetKey(KeyCode.C) && anim.GetInteger("up") == 4)
            {
            ...
            //RUN
            if (ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) && run < 30) run++;
            if (!ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) && run > 0) run--;
            anim.SetFloat("run", run);

            //MOVE     
            anim.SetFloat("walk", ControlFreak2.CF2Input.GetAxis("Vertical"));
            
            //TURN           
            anim.SetFloat("turn", ControlFreak2.CF2Input.GetAxis("Horizontal"));
            if (anim.GetFloat("walk") !=0f && grounded && active)trans.Rotate(0f, ControlFreak2.CF2Input.GetAxis("Horizontal") * 2f, 0f);                
            trans.Rotate(0f, anim.GetFloat("angle"), 0f);

            //JUMP & STAND UP
            if (ControlFreak2.CF2Input.GetButtonDown("Jump") && grounded && anim.GetInteger("up") == 4 && active)
            {
            ...
            if (ControlFreak2.CF2Input.GetButtonDown("Jump") && grounded && anim.GetInteger("up") == 3 && active)
            {
                if (anim.GetFloat("walk") == 0f) StartCoroutine("wait", waitime);
                Raycorrection = 0.025f;
                anim.SetInteger("up", 4);
    ...
    {             
        ...
        {
            //AIR MOVE
            if (!grounded && ControlFreak2.CF2Input.GetAxis("Vertical") >  0.01f) rigidbo.AddForce(trans.forward * 0.05f, ForceMode.Impulse);
            if (!grounded && ControlFreak2.CF2Input.GetAxis("Vertical") < -0.01f) rigidbo.AddForce(trans.forward * -0.05f, ForceMode.Impulse);
